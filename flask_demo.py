...
def upload_file():
    ...
    preview_dir = os.path.join(os.path.dirname(__file__), "preview_images")
    if not os.path.exists(preview_dir):
        os.makedirs(preview_dir)

    preview_file_paths = ppt_preview(
        filepath, os.path.join(preview_dir, generated_uuid + ".jpg")
    )
    ...
    if len(preview_file_paths) > 0:
        ...
        print(
            "Uploading preview images to S3: {:.2f}s".format(time.time() - start_time)
        )
